  ...
  return (
    <section className="border-t border-line bg-ivory py-16 md:py-20">
      <div className="container-editorial">
        ...
        <h2 className="mb-8 max-w-lg text-3xl text-charcoal sm:text-4xl md:mb-10 md:text-[2.75rem]">
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-7 sm:grid-cols-2 lg:grid-cols-4">
          {process.map((step, i) => (
            <div
              key={step.number}
              className={`border-t border-line pt-5 ${i > 0 ? "lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0" : ""}`}
            >
              <span className="font-display text-lg font-extrabold text-clay">{step.number}</span>
              <h3 className="mt-2.5 text-xl text-charcoal">{step.title}</h3>
